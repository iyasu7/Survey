      ...
      <Disclosure as="nav" class="bg-gray-800" v-slot="{ open }">
                ...
                <Menu as="div" class="relative ml-3">
                  <div>
                    <MenuButton class="flex max-w-xs items-center rounded-full bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
                      ...
                      <img class="h-8 w-8 rounded-full" :src="data.imageUrl" alt="" />
                    </MenuButton>
                  </div>
        ...
        <DisclosurePanel class="md:hidden">
          <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
            ...
            active-class="bg-gray-900 text-white"
             :class="[ route.name===item.to.name ? '' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'block px-3 py-2 rounded-md text-base font-medium']">{{ item.name }}</RouterLink>
          </div>
          <div class="border-t border-gray-700 pt-4 pb-3">
            <div class="flex items-center px-5">
              <div class="flex-shrink-0">
                <img class="h-10 w-10 rounded-full" :src="data.imageUrl" alt="" />
              </div>
              <div class="ml-3">
                <div class="text-base font-medium leading-none text-white">{{ data.name }}</div>
                <div class="text-sm font-medium leading-none text-gray-400">{{ data.email }}</div>
              </div>
            </div>
